...
//    PyObject * comps;
    ...
    if (PyTuple_Size(args) == 2 && PyArg_ParseTuple(args, "sO", &filename,
         &result)){
        FILE * fp = fopen (filename, "r");
        stat = readCalFile (fp, &pcomp);
        fclose (fp);
        if (stat.code == OK){
            pcal = Py_BuildValue ("k", (unsigned long*)pcomp);
//            PyList_Append (result, pcal);
            for (int i = 0; i < pcomp->ncomps; i++){
                nsum=0;
                index = 0;
                nprops = pcomp->comp[i]->nprops;
                ncomps = pcomp->comp[i]->ncomps;
                name = pcomp->comp[i]->name;
                cur = pcomp->comp[i]->prop;
                while (index < pcomp->comp[i]->nprops){
                ...
                if (nsum == 0){
                    PyList_Append (temp, Py_BuildValue ("(siis)", name, nprops, ncomps, ""));
                }
                else {
                    PyList_Append (temp, Py_BuildValue ("(siis)", name, nprops, ncomps, summary));
                }
            }
            PyList_Append (result, pcal);
...
PyObject *Cal_writeFile( PyObject *self, PyObject *args ){
    ...
    if (PyTuple_Size(args) == 3 && PyArg_ParseTuple( args, "skO", &filename,
         (unsigned long*)&pcal, &complist)){
        size = PyList_Size(complist);
        FILE * fp = fopen (filename, "w");
        if (size == 1){
            long index = PyLong_AsLong(PyList_GetItem (complist, 0));
            stat = writeCalComp (fp, pcal->comp[index]);
        }
        else if (size > 1){
            temp = malloc (sizeof (CalComp) + sizeof (CalComp*) * size);
            temp->name = pcal->name;
            temp->nprops = pcal->nprops;
            temp->prop = pcal->prop;
            temp->ncomps = size;
            for (int i = 0; i < size; i++){
                long index = PyLong_AsLong(PyList_GetItem (complist, i));
                temp->comp[i] = pcal->comp[index];
            }
            stat = writeCalComp (fp, temp);
            free (temp);
        }
        fclose (fp);
        return Py_BuildValue ("s", "OK");
